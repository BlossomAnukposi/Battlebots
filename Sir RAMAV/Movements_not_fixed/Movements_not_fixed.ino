...
void loop() {
    ...
    //prioritise right turns
    if (IR[0] > AVERAGE && IR[1] > AVERAGE && IR[6] > AVERAGE && IR[7] > AVERAGE) 
    {
        moveForward();
        delay(300);
        while (rightCount < PULSES_PER_90DEG_RIGHT)
        {
            turnRight();
    ...
    else if ((IR[6] > AVERAGE || IR[7] > AVERAGE) && IR[0] < AVERAGE) 
    {
        moveForward();
        delay(300);
        while (leftCount < PULSES_PER_90DEG_LEFT)
        {
            turnLeft();
    ...
    else if ((IR[0] > AVERAGE || IR[1] > AVERAGE) && IR[7] < AVERAGE) 
    {
        rightCount  =  0;
        moveForward();
        delay(300);
